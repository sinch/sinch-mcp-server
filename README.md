...
`sinchid-agent` credentials are resolved from an env var named `sinch-agent-m2m_<orderId>_<projectId>`, holding the same Base64 `projectId:keyId:keySecret` blob used by `client-credentials`. `orderId` comes from the `x-agent-id` header, and `projectId` is derived from the verified token's `https://sinch.com/project_id` claim. Both identifiers must contain only letters, digits, dots, or hyphens; underscores are rejected because they separate the identifiers. The decoded credential's project must match the verified project. A missing, malformed, mismatched, or ambiguously named value fails closed with a prompt response.
...
| Multi-tenant, `sinchid-agent`      | SinchID access token (three-segment JWT) | the agent installation (`sinch-agent-m2m_<orderId>_<projectId>` env var) |
