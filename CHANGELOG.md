...
- Added `MCP_AUTH_MODE`, which selects the tenancy of the HTTP server and is read before anything else:
  ...
  - `sinchid-agent` — **multi-tenant**. Callers send a SinchID access token plus an `x-agent-id` header. The token is cryptographically verified against JWKS (algorithm pinned to `RS256`, issuer, audience, and expiry), and the Sinch project id is derived from its verified `https://sinch.com/project_id` claim. Credentials are fetched at request time from the latest Google Secret Manager secret named `sinch-agent-m2m_<orderId>_<projectId>`; inaccessible, empty, malformed, or project-mismatched values fail closed. Requires `SINCHID_JWT_ISSUER`, `SINCHID_JWT_AUDIENCE`, `SINCHID_JWT_JWKS_URI`, and Google Application Default Credentials.
