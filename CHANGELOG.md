...
- Added `MCP_AUTH_MODE`, which selects the tenancy of the HTTP server and is read before anything else:
  ...
  - `sinchid-agent` — **multi-tenant**. Callers send a SinchID access token plus an `x-agent-id` header. Resolving credentials from the agent installation is not implemented yet (DEVEXP-1631), but the token itself is now cryptographically verified — signature against a JWKS (algorithm pinned to `RS256`), issuer, audience, and expiry — before the request is allowed through or any claim from it is trusted or logged. Requires `SINCHID_JWT_ISSUER`, `SINCHID_JWT_AUDIENCE` and `SINCHID_JWT_JWKS_URI`; the server refuses to start in this mode without all three.
