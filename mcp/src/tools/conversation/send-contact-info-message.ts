...
export const registerSendContactInfoMessage = (server: McpServer, tags: Tags[]) => {
  ...
  server.tool(
    ...
    'Send a contact info message to a recipient.',
    {
      ...
      contactPhoneNumbers: z.array(PhoneNumberModel).describe('Array of contact\'s phone numbers (E.164 format).'),
      contactEmailAddresses: z.array(EmailAddressModel).optional().describe('Array of contact\'s email addresses.'),
      contactAddresses: z.array(AddressModel).optional().describe('Array of contact\'s physical addresses.'),
      appId: ConversationAppIdOverride,
      sender: MessageSenderNumberOverride,
      region: ConversationRegionOverride,
    },
    sendContactInfoMessageHandler
  );
};

export const sendContactInfoMessageHandler = async ({
  recipient,
  channel,
  contactName,
  contactPhoneNumbers,
  ...
  contactName: z.infer<typeof NameModel>;
  contactPhoneNumbers: z.infer<typeof PhoneNumberModel>[];
  contactEmailAddresses?: z.infer<typeof EmailAddressModel>[];
  contactAddresses?: z.infer<typeof AddressModel>[];
  appId?: string;
  sender?: string;
  region?: string;
