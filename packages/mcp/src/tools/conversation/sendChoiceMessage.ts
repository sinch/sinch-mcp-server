...
export const registerSendChoiceMessage = (server: McpServer) => {
  server.tool(
    'send-choice-message',
    'Send a choice message to the user. The choice message can contain up to 10 choices. Each choice can be a call message (phone number + title to display next to it), a location message (latitude / longitude + title to display next to it), a text message or a URL message (the URL to click on + title to display next to it). The contact can be a phone number in E.164 format, or the identifier for the specified channel.',
    {
    ...
    }, async ({ contact, channel, choiceContent, text, appId, sender, region, sessionId }) => {
      ...
      const request: Conversation.SendChoiceMessageRequestData<Conversation.IdentifiedBy> = {
      ...
      let reply: string;
      try{
        response = await sinchClient.conversation.messages.sendChoiceMessage(request);
        reply = `Choice message submitted on channel ${channel}! The message ID is ${response.message_id}`;
      } catch (error) {
        reply = `An error occurred when trying to send the choice message: ${JSON.stringify(error)}. Are you sure you are using the right region to send your message? The current region is ${region}.`;
      }
